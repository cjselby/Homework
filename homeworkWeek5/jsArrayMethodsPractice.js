...
const websitesEmails = [];

data.forEach(curr => {
  let website = curr.website;
  let email = curr.email;
  websitesEmails.push([website, email]);
})

console.log(websitesEmails);
...
let toDos = moreData.filter(function(moreData) {
	return moreData.id == "9";
});

//then find the number of user 9's incomplete tasks

//Using the placeholder Post data at https://jsonplaceholder.typicode.com/pos
someMoreData = [
  {
    "userId": 1,
    "id": 1,
...
let posts = someMoreData.filter(function(someMoreData) {
	return someMoreData.id == "5";
});

//then, create a list of title and body pairs
const titleBody = [];

someMoreData.forEach(curr => {
  let title = curr.title;
  let body = curr.body;
  titleBody.push([title, body]);
})
